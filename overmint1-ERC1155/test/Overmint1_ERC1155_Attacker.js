...
describe(NAME, function () {
  ...
  describe("exploit", async function () {
    ...
    after(async function () {
      expect(
        await victimContract.balanceOf(attackerWallet.address, 0)
      ).to.be.equal(5);
      expect(
        await ethers.provider.getTransactionCount(attackerWallet.address)
      ).to.lessThan(4, "must exploit in three transactions or less");
      expect(
        await victimContract.success(attackerWallet.address, 0)
      ).to.be.true;
    });
  });
});
